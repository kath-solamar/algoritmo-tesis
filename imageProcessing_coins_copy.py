...
def addTags(conts, original_image):
    ...
    for contour in conts:
        ...
        if M["m00"] != 0:
            ...
            area = cv2.contourArea(contour)
            colorTag = (255, 0, 0)
            text = "ID:"+str(cont)+"-a:"+str(area)

            cv2.circle(original_image, (cX, cY), 5, (255, 0, 0), -1)
            cv2.putText(original_image, text, (cX - 40, cY - 40),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.3, colorTag, 1)
        else:
            cX, cY = 0, 0
...
def main():
    ...
    cv2.imwrite('coins-centroid.png', original_image)
