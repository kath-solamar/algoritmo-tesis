...
def imageEnhancement(image):
    ...
    ret, thresholded = cv2.threshold(gray, 210, 255, cv2.THRESH_BINARY_INV)
...
def imageCleaning(image):
    return cv2.morphologyEx(image, cv2.MORPH_OPEN, np.ones((6, 6)))
...
def main():
    ...
    gauss = imageGaussianBlur(resized_image)
    ...
    exportAreasToExcel(areas, "areas_output")
    exportPerimsToExcel(perims, "perimeters_output")
    exportEquiDiametersToExcel(ed, "diameters_output")
