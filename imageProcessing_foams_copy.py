...
def imageEnhancement(image):
    ...
    ret, thresholded = cv2.threshold(gray.copy(), 210, 255, cv2.THRESH_BINARY_INV)
...
def imageCleaning(image):
    return cv2.morphologyEx(image, cv2.MORPH_OPEN, np.ones((20, 20)))


# Detect the edges with Canny and Look for external contours inside the image
def getContours(image):
    
    borders = cv2.Canny(image, 0.1, 0.3)
...
def addTags(conts, original_image):
    cont = 0
    for contour in conts:
        # Calculate the contours moments to find their center
        ...
        if M["m00"] != 0:
            ...
            cv2.putText(original_image, "ID:"+str(cont)+"-a:"+str(area), (cX - 25, cY - 25),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, colorTag, 2)
        else:
            cX, cY = 0, 0

# Main function that executes the other functions
def main():
    original_image = readImage("./Images/30-D.tif")
    ...
    contours = getContours(cleaned_image)
    ...
    # Draw the resized image with green contours
    cv2.drawContours(original_image, contours, -1, (0, 255, 0), 3)
    addTags(contours, original_image=)
    showImage("Centroid", original_image)
    
    #Save the image in folder
    cv2.imwrite('centroid.png', original_image)

    # showImage("Resized image with draw Contours", resized_image)

    exportAreasToExcel(areas, "./exports/areas_output")
    exportPerimsToExcel(perims, "./exports/perimeters_output")
    exportEquiDiametersToExcel(ed, "./exports/diameters_output")
